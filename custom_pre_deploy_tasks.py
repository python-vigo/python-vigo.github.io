...
def generate_events_file():
    ...
    for last_post in sorted(posts_dir.glob('reunion_del_grupo_*'), reverse=True):
        # Get post metadata
        post_metadata = {}
        ...
        meeting_localtime_string = post_metadata.get('meeting_datetime')
        if not meeting_localtime_string:
            # Ignoramos el post si no tiene configurada la fecha de la reunión (p. ej. descansos de verano)
            continue
